...
function parseProductEntries(entries, cb = parseProduct) {
  const shuffledEntries = shuffleArray(entries?.items);

  return shuffledEntries?.map(cb)
}
...
export async function getRandomProduct(preview) {
  const entries = await getClient(preview).getEntries({
